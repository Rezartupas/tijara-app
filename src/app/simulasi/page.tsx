...
import ProductCard from "@/components/simulation/ProductCard";
import SimulationCalculator from "@/components/simulation/SimulationCalculator";
import SimulationBreakdown from "@/components/simulation/SimulationBreakdown";
import ManualProductForm from "@/components/submission/ManualProductForm";
...
function SimulationContent() {
  ...
  if (!url && !manual) {
    return (
      <section className="flex flex-col items-center text-center">
        <h1 className="text-xl font-semibold">Masukkan Tautan Produk</h1>
  ...
  if (loading) {
    return (
      <div className="space-y-6 animate-pulse" role="status" aria-label="Memuat data produk">
  ...
  if (error || (manual && !product)) {
    return (
      <section className="animate-fade-in-up flex flex-col items-center w-full">
...
export default function SimulasiPage() {
  return (
    <Suspense fallback={
      <div className="space-y-6 animate-pulse" role="status" aria-label="Memuat simulasi">
        <div className="h-40 w-full rounded-2xl bg-gray-200" />
        <div className="h-64 w-full rounded-2xl bg-gray-200" />
        <span className="sr-only">Memuat...</span>
      </div>
