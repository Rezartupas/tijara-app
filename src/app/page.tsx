...
export default function HomePage() {
  return (
    <section className="flex flex-col items-center text-center animate-fade-in-up py-4 sm:py-10">
      ...
      <h2 className="mt-2 sm:mt-4 text-4xl font-extrabold tracking-tight sm:text-6xl bg-gradient-to-r from-primary-800 via-primary-500 to-primary-600 bg-clip-text text-transparent pb-2 font-heading leading-tight px-2">
      ...
      <div className="mt-12 sm:mt-20 w-full">
        <h3 className="text-xl sm:text-2xl font-bold font-heading text-gray-800 mb-6 sm:mb-10">Cara Kerja Tijara</h3>
        <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-4 px-2 sm:px-0">
          {STEPS.map((step, i) => (
            <div
              key={step.label}
            ...
            >
              ...
              <div className="relative flex items-start sm:block gap-4 sm:gap-0">
                <span className="shrink-0 inline-flex items-center justify-center rounded-xl bg-gray-50 p-3 sm:p-4 text-2xl shadow-sm ring-1 ring-gray-100 transition-transform group-hover:scale-110 group-hover:bg-white group-hover:ring-primary-100">{step.icon}</span>
                <div className="mt-1 sm:mt-5">
                  <div className="flex items-center gap-2 mb-1 sm:mb-2">
                    <span className="inline-flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded-full bg-primary-100 text-[10px] sm:text-xs font-bold text-primary-700 shadow-inner">
                      {i + 1}
                    </span>
                    <p className="text-sm sm:text-base font-bold text-gray-900">{step.label}</p>
                  </div>
